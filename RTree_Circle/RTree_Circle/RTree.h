...
public:
	...
	public:
		...
		void GetBounds(ELEMTYPE a_min[DIMENSION], ELEMTYPE a_max[DIMENSION])
		{
			ASSERT(IsNotNull());
			StackElement& curTos = m_stack[m_tos - 1];
	...
	RTREE_QUAL::RTree()
{
	...
	m_root = AllocNode();
	m_root->m_level = 0;
	m_unitSphereVolume = (ELEMTYPEREAL)UNIT_SPHERE_VOLUMES[DIMENSION];
}
...
RTREE_TEMPLATE
	void RTREE_QUAL::Remove(const ELEMTYPE a_min[DIMENSION], const ELEMTYPE a_max[DIMENSION], const DATATYPE& a_dataId)
{
#ifdef _DEBUG
	for(int index=0; index<DIMENSION; ++index)
	{
...
double getDis(double centerA[DIMENSION], double centerB[DIMENSION])
{
	...
	{
		d += (centerA[i] - centerB[i]) * (centerA[i] - centerB[i]);
	}
	return sqrt(d);
}
